...
data = pd.read_csv("../Get_Comment/comments.csv",encoding="iso-8859-9",delimiter="\t")

#We round up comments with a score of 4 to 5, and comments with a score of 2 to 1.
data.loc[data['Puan'] == "5", 'Puan'] = 1.0
data.loc[data['Puan'] == "4", 'Puan'] = 1.0
data.loc[data['Puan'] == "2", 'Puan'] = 0.0
data.loc[data['Puan'] == "1", 'Puan'] = 0.0

#We create new data frame for 5 and 1
dataFrame1 = data[data.Puan == 1.0]
dataFrame0 = data[data.Puan == 0.0]

#We concat dataFrame5 and dataFrame1 with concat command. We ignore old index with ignore_index=true
df = pd.concat([dataFrame0, dataFrame1], ignore_index=True)
...
df.to_csv('comments_final.csv', sep="\t", encoding="iso-8859-9", index=False)